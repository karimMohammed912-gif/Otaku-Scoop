...
import 'manga_details_tab.dart';
import 'manga_chapters_tab.dart';
import 'characters_tab.dart';

class MangaContent extends StatelessWidget {
  final dynamic media; // details_root.Media
  const MangaContent({super.key, required this.media});
  ...
  Widget build(BuildContext context) {
    ...
    final chapters = media.chapters ?? 0;
    final volumes = media.volumes ?? 0;
    final status = media.status ?? 'Unknown';
    final genres = (media.genres as List<String>?) ?? const <String>[];
    final score = media.averageScore ?? 0;
    final startedAt = media.startDate ?? null;
    final endedAt = media.endDate ?? null;
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Wrap(
                                  ...
                                  runSpacing: 8,
                                  children: [
                                    StatChip(icon: Icons.menu_book_rounded, label: '$chapters ch'),
                                    StatChip(icon: Icons.library_books_rounded, label: '$volumes vol'),
                        ...
                        child: TabBarView(
                          children: [
                            MangaDetailsTab(
                              ...
                              status: status,
                              genres: genres,
                              averageScore: score, startedAt:startedAt , endedAt:endedAt,
                            ),
                            MangaChaptersTab(
                              chapters: chapters,
                              volumes: volumes,
